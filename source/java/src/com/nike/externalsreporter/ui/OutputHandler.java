...
/**
 * Created by IntelliJ IDEA. User: douglasbullard Date: Jun 17, 2008 Time: 10:10:22 PM To change this template use File | Settings | File
 * Templates.
 */
...
public class OutputHandler
{
    ...
    private void writeDotFileTargetDeclarations(DataOutputStream out,
                                                Map<BuildableProjects, Map<String, List<External>>> dependencies) throws IOException
    {
        int clusterIndex = 0;
        ...
        for (BuildableProjects project : projectsSet)
        {
            ...
            for (String branchName : branchesExternals.keySet())
            {
                ...
                for (External external : externals)
                {
                    ...
                    String line = "\t\t" + QUOTE + filteredName + QUOTE + " [label=" + QUOTE + filteredName + QUOTE + " shape=" + "box" +
                                  " color=" + "black" + " ]; ";
    ...
    public File launchDot(File dotFile, File dotExecutable) throws IOException, InterruptedException
    {
        ...
        String[] command =
                {dotExecutable.getAbsolutePath(), "-T" + mainFrame.getOs().getOutputFormat().getType(), dotFilePath, "-o" + outputFilePath};
