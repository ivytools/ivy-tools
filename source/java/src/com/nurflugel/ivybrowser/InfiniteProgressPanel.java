...
/**
 * An infinite progress panel displays a rotating figure and a message to notice the user of a long, duration unknown task. The shape and the text are drawn upon a white veil which alpha level (or shield value) lets the underlying component shine through. This panel is meant to be used asa <i>glass
 * pane</i> in the window performing the long operation.<br />
 * <br />
 * On the contrary to regular glass panes, you don't need to set it visible or not by yourself. Once you've started the animation all the mouse events are intercepted by this panel, preventing them from being forwared to the underlying components.<br />
 * <br />
 ...
 * <br />
 * <br />
 * Several properties can be configured at creation time. The message and its font can be changed at runtime. Changing the font can be done using <code>setFont()</code> and <code>setForeground()</code>.
 *
 * @author   Romain Guy
 * @version  1.0
 */
...
public class InfiniteProgressPanel extends JComponent implements MouseListener
{
    ...
    private static final long serialVersionUID = 1509217217194094951L;
    ...
    public InfiniteProgressPanel(String text, int barsCount, float shield, float fps, int rampDelay)
    {
        ...
        this.shield    = (shield >= 0.0f) ? shield
                         : 0.0f;
        this.fps       = (fps > 0.0f) ? fps
                         : 15.0f;
        this.barsCount = (barsCount > 0) ? barsCount
                         : 14;
    ...
    @Override @SuppressWarnings({ "NumericCastThatLosesPrecision", "RefusedBequest" })
    ...
    public void start()
    {
        System.out.println("Starting progress pane");
    ...
    public void stop()
    {
        System.out.println("Stopping progress pane");
