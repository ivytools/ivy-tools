...
//        clientManager = SVNClientManager.newInstance(options, WebAuthenticator.getUsername(), WebAuthenticator.getPassword());
    ...
    /*
     * Duplicates srcURL to dstURL (URL->URL)in a repository remembering history. Like 'svn copy srcURL dstURL -m "some comment"' command. It's done by invoking
     *
     ...
     *
     * dstURL - a URL where srcURL will be copied; if srcURL & dstURL are both directories then there are two cases: a) dstURL already exists - then doCopy(..) will duplicate the entire source directory and put it inside dstURL (for example, consider srcURL = svn://localhost/rep/MyRepos, dstURL =
     * svn://localhost/rep/MyReposCopy, in this case if doCopy(..) succeeds MyRepos will be in MyReposCopy - svn://localhost/rep/MyReposCopy/MyRepos); b) dstURL doesn't exist yet - then doCopy(..) will create a directory and recursively copy entries from srcURL into dstURL (for example, consider the
     * same srcURL = svn://localhost/rep/MyRepos, dstURL = svn://localhost/rep/MyReposCopy, in this case if doCopy(..) succeeds MyRepos entries will be in MyReposCopy, like: svn://localhost/rep/MyRepos/Dir1 -> svn://localhost/rep/MyReposCopy/Dir1...);
     *
     * isMove - if false then srcURL is only copied to dstURL what corresponds to 'svn copy srcURL dstURL -m "some comment"'; but if it's true then srcURL will be copied and deleted - 'svn move srcURL dstURL -m "some comment"';
     *
